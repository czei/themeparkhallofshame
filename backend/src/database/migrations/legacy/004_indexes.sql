...
-- Updated: 2025-12-16 - Made idempotent with DROP INDEX IF EXISTS

-- Additional indexes for park filtering (Disney & Universal filter)
DROP INDEX IF EXISTS idx_parks_filter_active ON parks;
...
DROP INDEX IF EXISTS idx_rides_park_tier_active ON rides;
CREATE INDEX idx_rides_park_tier_active
    ON rides(park_id, tier, is_active);

-- Covering index for current wait times query (FR-017)
DROP INDEX IF EXISTS idx_snapshots_current_status ON ride_status_snapshots;
...
DROP INDEX IF EXISTS idx_ride_daily_ranking ON ride_daily_stats;
CREATE INDEX idx_ride_daily_ranking
    ON ride_daily_stats(stat_date DESC, downtime_minutes DESC);

DROP INDEX IF EXISTS idx_ride_weekly_ranking ON ride_weekly_stats;
...
DROP INDEX IF EXISTS idx_classifications_tier_weight ON ride_classifications;
...
DROP INDEX IF EXISTS idx_aggregation_cleanup ON aggregation_log;
...
DROP INDEX IF EXISTS idx_park_activity_detection ON park_activity_snapshots;
